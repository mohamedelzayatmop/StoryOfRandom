...
namespace StoryOfRandom.ScriptableObjects
{
    ...
    public class RandomColors : ScriptableObject, IRandom
    {
        ...
        private void RandomOffset()
        {
            ...
            for (int i = 0; i < SquareSize.x; i++)
            {
                for (int j = 0; j < SquareSize.y; j++)
                {
                    ...
                    var InstantiatedObject = Instantiate(Prefab, new Vector3(i - (SquareSize.x / 2), 0, j - (SquareSize.y / 2)), Quaternion.identity);
                    var ObjectRenderer = InstantiatedObject.GetComponent<MeshRenderer>();
                    
                    ObjectRenderer.GetPropertyBlock(ColorPropertyBlock);
                    ColorPropertyBlock.GetColor(MainColor);
                    
                    var Value = (Color1.r + Color1.g + Color1.b)/3;
                    var NewValue = Value + (2 * Random.Range(RandomRange.x, RandomRange.y)) * Offset - Offset;
                    var ValueRatio = NewValue / Value;
                    var NewColor = new Color(Color1.r * ValueRatio, Color1.g * ValueRatio, Color1.b * ValueRatio);
        ...
        private void GoldenRatioWithGradient()
        {
            ...
            for (int i = 0; i < SquareSize.x; i++)
            {
                for (int j = 0; j < SquareSize.y; j++)
                {
                    ...
                    var NewColor = ColorGradient.Evaluate(Random.Range(-0.5f, 0.5f) + (1.618033988749895f * j) % 1);  
